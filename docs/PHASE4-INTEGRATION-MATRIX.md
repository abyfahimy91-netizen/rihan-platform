...
>
> ⚠️ **یادداشت بازنویسی (D-079):** در تاریخ ۲۰۲۶-۰۸-۱۳، ۴ سند ADR (ADR-002, ADR-003, ADR-007, ADR-008) بر اساس D-079 به‌روزرسانی شدند. بخش‌های جدید شامل:
> - سیستم بلوک‌محور (ContentBlock + ProductBlock)
> - سئو فنی (Meta tags, Schema.org, sitemap.xml)
> - قوانین برند مستقل در CI/CD
...
## ۱۰. یکپارچگی D-079 با اسناد فاز ۴

این بخش، یکپارچگی تغییرات اعمال‌شده بر اساس D-079 را بین ۴ سند به‌روزرسانی‌شده تأیید می‌کند.

### ۱۰.۱ سیستم بلوک‌محور (ADR-002 + ADR-003 + ADR-007)

| لایه | سند | نقش در سیستم بلوک‌محور |
| --- | --- | --- |
| داده | ADR-002 | موجودیت ContentBlock + ProductBlock + فیلدهای سئو در Product |
| API | ADR-003 | Endpoints برای CRUD بلوک‌ها، reorder، و SEO |
| فرانت‌اند | ADR-007 | Alpine.js component برای رندر بلوک‌ها + HTMX lazy loading |

**مسیر داده:**
1. ادمین بلوک را در پنل ایجاد می‌کند (ADR-003: POST /api/content-blocks)
2. بلوک در دیتابیس ذخیره می‌شود (ADR-002: content_blocks table)
3. ادمین بلوک را به محصول اختصاص می‌دهد (ADR-003: POST /api/products/:id/blocks)
4. رابطه در ProductBlock ذخیره می‌شود (ADR-002: product_blocks table)
5. کاربر محصول را می‌بیند (ADR-003: GET /api/products/:id/blocks)
6. Alpine.js بلوک‌ها را رندر می‌کند (ADR-007: productBlocks component)

**تأیید یکپارچگی:** ✅ هیچ تناقضی بین سه سند وجود ندارد.

---

### ۱۰.۲ سئو فنی (ADR-002 + ADR-003 + ADR-007 + ADR-008)

| لایه | سند | نقش در سئو فنی |
| --- | --- | --- |
| داده | ADR-002 | فیلدهای seo_title, seo_description, seo_keywords در Product |
| API | ADR-003 | Endpoints SEO API + Sitemap/Robots API |
| فرانت‌اند | ADR-007 | Meta tags, Schema.org, Open Graph |
| استقرار | ADR-008 | sitemap.xml, robots.txt در Nginx + Cron job |

**مسیر تولید sitemap:**
1. Cron job روزانه (ADR-008) trigger می‌شود
2. Django command محصولات active را از دیتابیس می‌خواند (ADR-002)
3. sitemap.xml تولید و در /static ذخیره می‌شود
4. Nginx فایل را سرو می‌کند (ADR-008)

**مسیر سئو محصول:**
1. ادمین فیلدهای سئو را ویرایش می‌کند (ADR-003: PUT /api/products/:id/seo)
2. داده در دیتابیس ذخیره می‌شود (ADR-002: seo_* fields)
3. فرانت‌اند meta tags تولید می‌کند (ADR-007: seoData component)
4. Schema.org JSON-LD به‌صورت خودکار تولید می‌شود

**تأیید یکپارچگی:** ✅ چهار سند کاملاً هماهنگ هستند.

---

### ۱۰.۳ قوانین برند مستقل (ADR-007 + ADR-008)

| لایه | سند | نقش در برند مستقل |
| --- | --- | --- |
| فرانت‌اند | ADR-007 | قوانین قالب‌بندی + فوتر عمومی |
| استقرار | ADR-008 | CI/CD تست خودکار قبل از deploy |

**مسیر محافظت از برند:**
1. توسعه‌دهنده قالب را تغییر می‌دهد
2. Git push به repository
3. GitHub Actions تست برند مستقل را اجرا می‌کند (ADR-008)
4. اگر کلمه ممنوعه یافت شود، deploy رد می‌شود
5. در غیر این صورت، deploy ادامه می‌یابد
6. فرانت‌اند قوانین فوتر را رعایت می‌کند (ADR-007)

**تأیید یکپارچگی:** ✅ دو لایه حفاظتی (تست + اجرای قوانین).

---

### ۱۰.۴ ماتریس کامل تغییرات D-079

| تغییر D-079 | ADR-002 | ADR-003 | ADR-007 | ADR-008 |
| --- | :---: | :---: | :---: | :---: |
| سیستم بلوک‌محور | ✅ ContentBlock + ProductBlock | ✅ Block APIs | ✅ Alpine رندر | — |
| فیلدهای سئو | ✅ seo_* fields | ✅ SEO API | ✅ Meta tags | — |
| sitemap.xml + robots.txt | — | ✅ Sitemap API | — | ✅ Nginx + Cron |
| قوانین برند مستقل | — | — | ✅ فوتر | ✅ CI/CD تست |

**نتیجه:** تمام تغییرات D-079 در اسناد مربوطه اعمال شده و یکپارچگی ۱۰۰٪ تأیید می‌شود.

---

### ۱۰.۵ چک‌لیست یکپارچگی D-079

- [x] ADR-002: فیلدهای سئو و موجودیت‌های بلوک‌محور
- [x] ADR-003: APIهای جدید (ContentBlock, ProductBlock, SEO, Sitemap)
- [x] ADR-007: سئو فنی، رندر بلوک‌محور، قوانین برند مستقل
- [x] ADR-008: sitemap.xml، robots.txt، CI/CD تست برند
- [x] هیچ تناقضی بین اسناد وجود ندارد
- [x] مسیرهای داده end-to-end کامل هستند
- [x] تمام اسناد به D-079 ارجاع می‌دهند
