...
def tracking_page_view(request, order_number):
    ...
    # D-105: مرسوله‌های سفارش — هر بسته کد رهگیری و لینک پیگیری یک‌کلیکی خودش را دارد
    shipments = [
        s for s in order.shipments.prefetch_related('items__order_item')
...
# ═════════════════════════════════════════════════════════════
# D-105 — لینک رهگیری یک‌کلیکی: /order/t/<کد>/
# پیامک مشتری این لینک کوتاه را دارد؛ با باز شدن مستقیم به سامانه
# باربری (پست/تیپاکس/...) با کدِ پرشده ریدایرکت می‌شود — صفر اصطکاک.
# ═════════════════════════════════════════════════════════════

def tracking_redirect_view(request, code):
    normalized = ''.join(
        ch.upper() if ch.isalpha() else ch
        for ch in _to_en_digits(code) if ch.isalnum()
    )
    shipment = (
        Shipment.objects.filter(tracking_code=normalized)
    ...
    if not shipment:
        messages.warning(request, 'کد رهگیری در سیستم ما پیدا نشد. از شماره سفارش استفاده کنید.')
        return redirect('order_pages:tracking_lookup')

    url = shipment.tracking_url
    if not url:
        # شرکت حمل بدون لینک مستقیم → صفحه پیگیری خود سفارش
        return redirect('order_pages:tracking_page', order_number=shipment.order.order_number)
    return redirect(url)
