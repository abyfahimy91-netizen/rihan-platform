...
class ProductAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ('📣 متن اشتراک‌گذاری اختصاصی', {
            'fields': ('share_text',),
            'description': 'خالی = از تنظیمات سایت. با {min_price} ارزان‌ترین بسته را تزریق کن. دعوت به اقدام + کد تخفیف = فروش بیشتر.',
            'classes': ('collapse',),
        }),
        ...
        ('⚡ پاسخ سریع (GEO — هوش مصنوعی)', {
            'classes': ('collapse',),
            'description': 'کادر «پاسخ سریع» بلافاصله بعد از باکس خرید؛ ۲-۳ جملهٔ فکت‌محور که چت‌جی‌پی‌تی/پرپلکسیتی/گمینای آن را مستقیم نقل می‌کنند. همان قواعد متن.',
            'fields': ('geo_answer',),
        }),
        ('🎯 فیلتر شفاف مخاطب', {
            'classes': ('collapse',),
