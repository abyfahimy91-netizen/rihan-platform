...
class Product(models.Model):
    ...
    result_headline = models.CharField(
        "تیتر نتیجه‌محور (بالای نام محصول)", max_length=180, blank=True, default='',
        help_text='مثلاً: «دیگر نگران تقلبی بودن سماق نباشید» — اگر خالی باشد نمایش داده نمی‌شود.',
    )
    compare_at_price = models.DecimalField(
        "قیمت قبل از تخفیف (خط‌خورده)", max_digits=10, decimal_places=2,
        null=True, blank=True,
        help_text='اگر بزرگ‌تر از قیمت فعلی باشد، به‌صورت خط‌خورده + درصد تخفیف نمایش داده می‌شود.',
    )
    fit_for = models.TextField(
        "این کالا برای شماست اگر...", blank=True, default='',
        help_text='هر خط یک مورد؛ خط‌ها با «-» شروع شوند فهرست می‌شوند. خالی = بخش مخفی.',
    )
    not_fit_for = models.TextField(
        "لطفاً سفارش ندهید اگر...", blank=True, default='',
        help_text='هر خط یک مورد؛ صادقانه بنویسید چه کسانی این کالا برایشان مناسب نیست.',
    )
    curation_story = models.TextField(
        "داستان گزینش و آزمون کیفیت", blank=True, default='',
        help_text='قواعد متن: خط خالی=پاراگراف، #=تیتر، -=فهرست، ۱.=شماره‌ای، >=نقل‌قول. چطور ده‌ها نمونه تست شد و چرا این انتخاب شد؟',
    )
    deep_dive = models.TextField(
    ...
    # SHARE-CTA-14050611: متن اختصاصی پیام اشتراک‌گذاری این محصول
    share_text = models.TextField(
        'متن اختصاصی اشتراک‌گذاری',
        blank=True, default='',
        help_text='خالی = متن عمومی از تنظیمات سایت. جای‌نگه‌دار {min_price} = ارزان‌ترین بسته. '
                  'بعد از این متن، نام محصول، لینک کوتاه و هشتگ‌ها خودکار اضافه می‌شود.',
    )
...
class ProductFaq(models.Model):
    """سوال متداول اختصاصی هر محصول — بخش ۶ صفحه فروش (D-104)"""
