...
## 1405/06/02 — D-099: مهلت رزرو + لغو مشتری + پروفایل جدید
- رفع باگ خط استپر: right:50% → left:50% در checkout_steps.html و tracking_page.html (RTL — خط آخر از کادر بیرون می‌زد و بین ۱و۲ نبود)
- Order.expires_at (migration 0010 + backfill)؛ پیش‌فرض ۶۰ دقیقه (ORDER_PAYMENT_TTL_MINUTES)
- آزادسازی: lazy در صفحات پرداخت/پیگیری/پروفایل + cron هر ۵ دقیقه (manage.py release_expired_orders)
- محافظ: سفارش با رسید در بررسی (PENDING_REVIEW) هرگز خودکار لغو نمی‌شود؛ ثبت رسید = تمدید ۴۸ ساعته
- لغو مشتری: POST /order/payment/<num>/cancel/ — فقط مالک، فقط PENDING، رسید‌دار ممنوع
- شمارش معکوس فارسی mm:ss در پرداخت/پیگیری/پروفایل؛ زیر ۵ دقیقه قرمز؛ صفر=reload
- پروفایل بازسازی شد: هدر آماری + تب‌ها (سفارش‌ها/حساب/امنیت/دستگاه‌ها) + کارت سفارش با اکشن وضعیت‌محور
- دام: header/footer تگ داخل کارت = ارث‌بری استایل سراسری base.html → div بزن؛ انیمیشن پنل = خرابی paint اسکرین‌شات WebKit
- ۱۵۳ تست سبز (۹ تست جدید)؛ سفارش واقعی RH-1405-00001 مهلتش ۲۴ ساعت تمدید شد (تصمیم با کاربر)
