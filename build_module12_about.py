...
# Update src/apps/core/views.py to add about_view
...
"""
if "def about_view" not in views_text:
    views_text += about_view_code
    views_file.write_text(views_text, encoding="utf-8")
    print("✓ Added about_view to src/apps/core/views.py")

# Update src/apps/core/urls.py to add /about/
urls_file = BASE / "src/apps/core/urls.py"
urls_text = urls_file.read_text(encoding="utf-8")
if "name='about'" not in urls_text:
    ...
    print("✓ Registered /about/ in src/apps/core/urls.py")

# Create Template: src/templates/core/about.html
about_template_file = BASE / "src/templates/core/about.html"
about_template_content = """{% extends 'base.html' %}
...
{% endblock %}
"""
about_template_file.write_text(about_template_content, encoding="utf-8")
print("✓ Created src/templates/core/about.html")

# Update base.html to add About Link in Navbar
base_template = BASE / "src/templates/base.html"
base_content = base_template.read_text(encoding="utf-8")
if '/about/' not in base_content:
    base_content = base_content.replace(
        '<a href="/products/" class="hover:text-rihan-600 transition">کاتالوگ محصولات</a>',
        '<a href="/products/" class="hover:text-rihan-600 transition">کاتالوگ محصولات</a>\n                        <a href="/about/" class="hover:text-rihan-600 transition">داستان اصالت ریهان</a>'
    )
    base_template.write_text(base_content, encoding="utf-8")
    print("✓ Added /about/ link to Navbar in base.html")

# Create Unit Tests: tests/test_about_page.py
...
class AboutPageTestCase(TestCase):
    def test_about_page_rendering_and_pillars(self):
        ...
        # Verify Independent Brand Rule (No founder personal full names in public site)
        self.assertNotContains(res, "عبدالحسین فهیمی")
"""
test_file.write_text(test_code, encoding="utf-8")
print("✓ Created tests/test_about_page.py")

# Register M12 in PluginRegistry
plugins_file = BASE / "src/apps/core/plugins.py"
plugins_text = plugins_file.read_text(encoding="utf-8")
if 'PluginRegistry.register("M12"' not in plugins_text:
    plugins_text += '\nPluginRegistry.register("M12", "صفحه اصالت و داستان برند ریهان Central Story", "0.5.12", is_system=True)\n'
    plugins_file.write_text(plugins_text, encoding="utf-8")
    print("✓ Registered M12 in PluginRegistry")

print("All Module M12 Files Deployed Successfully.")
