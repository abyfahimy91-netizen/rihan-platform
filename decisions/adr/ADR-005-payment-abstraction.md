...
> **الزام:** پیاده‌سازی Gateway در فاز ۵ باید کاملاً مطابق این ADR باشد.
...
- ورودی: پرداخت، دلیل
- خروجی: وضعیت refunded (یکدست با ENUM اصلی: pending/confirmed/rejected/refunded)
- برای CardToCard MVP: ثبت درخواست مرجوعی دستی + تغییر وضعیت به refunded (بدون وضعیت میانی refund_requested)
- برای درگاه آنلاین (آینده): می‌تواند دو مرحله‌ای باشد اما وضعیت نهایی Payment همان refunded است
...
**قانون کلیدی:** فقط تغییر Order.status باعث InventoryTransaction می‌شود؛ تغییر Payment.status به‌صورت مستقیم موجودی را عوض نمی‌کند، مگر اینکه به تأیید/لغو Order منجر شود. این هم‌راستایی کامل با ADR-002 است.
...
این سند اکنون **مصوب** است و لایه انتزاع پرداخت برای MVP ریهان قطعی شد.
