...
| مرتبط | ADR-001, ADR-003, ADR-003-Appendix, ADR-006, ADR-004, CENTRAL-STORY.md, اصل ۹, ۱۰, ۱۱, **D-079** |

> ⚠️ **یادداشت بازنویسی (D-079):** در تاریخ ۲۰۲۶-۰۸-۱۳، سه تغییر اساسی بر اساس D-079 اعمال شد:
> - افزودن **سئو فنی** (Meta tags, Schema.org, Open Graph, Twitter Cards)
> - افزودن **رندر بلوک‌محور** برای روایت محصول
> - افزودن **قوانین برند مستقل** در فرانت‌اند
...
## ۹. سئو فنی در فرانت‌اند (D-079)

### ۹.۱ Meta Tags (SEO Technical)

هر صفحه محصول و صفحه درباره باید دارای meta tags کامل باشد:
...
### ۹.۲ Schema.org Structured Data (JSON-LD)

برای هر صفحه محصول، JSON-LD به‌صورت خودکار تولید می‌شود:

- Product Schema: name, image, description, offers
- AboutPage Schema: برای صفحه درباره
- LocalBusiness Schema: سئو محلی (D-048)

### ۹.۳ سئو محلی (Local SEO - D-048)

- LocalBusiness Schema با آدرس و مناطق تحت پوشش
- مناطق: هوراند، تبریز، تهران
- ساعات کاری و اطلاعات تماس

### ۹.۴ تولید خودکار در Alpine.js

یک component سراسری Alpine.js برای تولید meta tags به‌صورت داینامیک.

---

## ۱۰. رندر بلوک‌محور در صفحه محصول (D-079)

### ۱۰.۱ ساختار رندر

صفحه محصول با استفاده از Alpine.js و HTMX بلوک‌ها را به‌صورت داینامیک رندر می‌کند. هر بلوک بر اساس type خود رندر می‌شود:

- heading: عنوان H2/H3/H4
- text: پاراگراف با alignment
- image: عکس با alt_text و caption
- gallery: گالری عکس با ۲/۳/۴ ستون
- video: ویدیو (آپلود یا لینک)
- quote: نقل قول با author
- table: جدول با header
- cta: دکمه اقدام
- trust_badges: نشانه‌های اعتماد
- related_products: محصولات مرتبط

### ۱۰.۲ Alpine Component برای بلوک‌ها

یک Alpine component برای بارگذاری بلوک‌ها از API و رندر آن‌ها در صفحه محصول.

### ۱۰.۳ HTMX برای Lazy Loading

بلوک‌های سنگین (ویدیو، گالری) با HTMX به‌صورت lazy بارگذاری می‌شوند تا صفحه سریع لود شود.

---

## ۱۱. قوانین برند مستقل در فرانت‌اند (D-079)

### ۱۱.۱ کلمات ممنوعه در قالب‌ها

هیچ قالبی نباید شامل کلمات زیر باشد:
- عبدالحسین فهیمی
- فهیمی
...
هیچ تصویر شخصی بنیان‌گذار در صفحات عمومی بارگذاری نشود.

### ۱۱.۳ تست خودکار در CI/CD

قبل از هر deploy، قالب‌های HTML اسکن می‌شوند. در صورت یافت شدن کلمات ممنوعه، deploy رد می‌شود.

### ۱۱.۴ فوتر عمومی

فوتر فقط شامل نام برند «ریهان»، لینک‌های اصلی، و اطلاعات تماس است. هیچ اشاره‌ای به بنیان‌گذار نیست.

### ۱۱.۵ پنل ادمین

پنل ادمین می‌تواند نام ادمین‌ها را ببیند، اما سایت عمومی هرگز نباید این اطلاعات را نمایش دهد.

---

## ۱۲. Out of Scope صریح
...
## ۱۳. هم‌راستایی با سایر ADRها و اصول
