...
**تاریخ تصویب:** ۱۴۰۵/۰۵/۱۶  
**تأییدکننده:** مشاور پروژه  
**ارجاع:** تأیید رسمی ADR-002 v3 با ۵ اصلاح نهایی

### خلاصه تأیید:
- ✅ Guest Checkout + shipping_* روی Order
- ✅ Address.user_id نال‌پذیر
- ✅ D-068 ثبت رسمی
- ✅ حذف PriceHistory ناقص
- ✅ Payment مطابق D-067 + چند تلاش
- ✅ RBAC / Cart / Inventory GENERATED / 1:1

این سند اکنون **مصوب** است و مبنای اجرایی برای شروع ADR-003 (API Strategy) می‌باشد.

### نکته غیرمسدودکننده:
در Payment، فیلد card_number مقصد برای MVP قابل قبول است. بهتر است در ADR-005 (Payment Abstraction) حساب مقصد از تنظیمات سیستم خوانده شود و در هر ردیف Payment تکرار نشود.
